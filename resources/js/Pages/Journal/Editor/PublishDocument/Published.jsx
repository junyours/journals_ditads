...
const Published = () => {
  ...
  const handleSearch = (e) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      router.get(route('editor.published.document.report'), { search: value }, { preserveState: true });
    }, 1000);
  };
            ...
            {assigns.data.length > 0 ? (
              assigns.data.map((assign, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    {assign.published_file.split('/').pop()}
                  </TableCell>
                  <TableCell>
                    {formatDateTime(assign.published_at)}
                  </TableCell>
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => window.open(route('journal.download', { file: `published_files/${assign.published_file}` }), '_blank')} className="cursor-pointer">
